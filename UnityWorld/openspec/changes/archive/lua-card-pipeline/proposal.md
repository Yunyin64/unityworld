...
卡牌系统的 Lua 脚本执行链路当前是断的：`CombatCard` 已有 `env` 字段和 `Contest`/`Apply` 占位，`LuaMgr` 已有脚本加载和 Hook 调用接口，但两者之间没有真正连通。需要跑通完整的 Lua 调用管线，让卡牌脚本可以定义战斗行为。
...
- `CombatCard.Contest()` / `Apply()` 构造 `APIContext` 并调用 Lua 函数
...
- `Data/LuaCards/card_form_quan_da.lua` 补全 Contest 实际逻辑

## Capabilities

### New Capabilities
- `lua-card-runtime`: 卡牌 Lua 脚本运行时管线——从加载、实例化到 Contest/Apply 的完整调用链路
...
- `Scripts/Game/Domain/Combat/CombatCard/CombatCard.cs` — Contest/Apply 填入 Lua 调用
