...
**Goals:**
- 引入 `Scope` 枚举分层，支持 `Global / Npc / Tile / Plane` 范围的精准广播
...
**选择**：`enum Scope { Global, Npc, Tile, Plane, ... }`  
...
1. 新增 `EventDefine.cs`、`EventDefineMgr.cs`、`Scope` 枚举、`ScopeKey`、`IEventListener`
...
- `Scope` 后续是否需要 `Faction`、`Combat` 等新 scope？ → 按需添加枚举值即可，无破坏性变更
