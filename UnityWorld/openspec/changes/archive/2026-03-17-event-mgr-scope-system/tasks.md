...
- [x] 1.1 在 `Scripts/Game/Data/Enum/EnumTypes.cs` 中添加 `Scope` 枚举（`Global, Npc, Tile, Plane`），并将 `Em_Event` 枚举标记 `[Obsolete]`
...
- [x] 2.1 新建 `Scripts/Game/Data/Defines/EventDefine.cs`，继承 `DefineBase`，字段：`Scope[] Scopes`（JSON 反序列化）
...
- [x] 3.4 实现 `TriggerEvent(string eventId, object args, params (Scope scope, string id)[] scopes)`：自动补充 Global 广播、遍历各 scope 监听列表、悬空监听自动清理 + Warn、EventDefine scope 声明校验 Warn
