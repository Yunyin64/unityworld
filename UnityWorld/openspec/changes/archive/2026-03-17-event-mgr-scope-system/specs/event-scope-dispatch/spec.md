...
### Requirement: Scope 枚举定义广播层级
系统 SHALL 提供 `Scope` 枚举，包含 `Global`、`Npc`、`Tile`、`Plane` 等游戏内有限实体类型作为广播层级。后续按需可扩展新枚举值，不破坏现有逻辑。
...
系统 SHALL 提供 `ScopeKey` 值类型（struct），包含 `Scope Scope` 和 `string Id` 两个字段。`Global` scope 时 `Id` 为空字符串 `""`。`ScopeKey` SHALL 正确实现值相等（`Equals`/`GetHashCode`），以保证在 Dictionary 中作键时行为正确。

#### Scenario: Global ScopeKey 的标准创建
- **WHEN** 创建 `ScopeKey.Global`
- **THEN** 等价于 `new ScopeKey(Scope.Global, "")`

#### Scenario: 相同 Scope 和 Id 的两个 ScopeKey 相等
- **WHEN** `new ScopeKey(Scope.Npc, "42") == new ScopeKey(Scope.Npc, "42")`
...
- **WHEN** `EventMgr.RegisterEvent("npc_moved_listener", "NpcMoved", new ScopeKey(Scope.Npc, "42"), listener)` 被调用
...
`EventMgr.TriggerEvent` SHALL 接受 `string eventId`、`object args`、以及 `params (Scope scope, string id)[]` scope 列表，依次向 Global scope 和所有传入 scope 对应的监听者列表广播事件。触发时 SHALL 自动补充 Global scope 广播（无需调用方显式传入）。
