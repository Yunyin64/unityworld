...
- [x] 1.4 创建 `Scripts/Game/Data/Defines/ActionCardDefine.cs`：继承 DefineBase，含 Tags、StoryIds、StoryTags、IsConsumable 字段
...
- [x] 1.7 创建 `Scripts/Game/Data/Mgr/ActionCardDataMgr.cs`：实现 IDataMgrBase\<ActionCardDefine\>，加载 JSON
- [x] 1.8 在 `GameDataMgr` 构造函数中注册 StoryDefineMgr、OptionDefineMgr、ActionCardDataMgr
...
- [x] 2.5 创建 `Scripts/Game/Domain/Story/StoryEffectFunc.cs`：静态类，维护 `Dictionary<string, Action<StoryContext, List<string>>>` 注册表，内置所有原子效果函数（GiveTrait/RemoveTrait/GiveActionCard/ModifyAura/TriggerStory/TriggerStoryByTag/AddToFatePool/AddToKarmaPool/ModifyStat/EmitEvent），提供 `Register` 和 `Execute` 方法
...
- [x] 5.1 创建 `Scripts/Game/Domain/ActionCard/ActionCard.cs`：运行时实例类，含 DefineId、OwnerId(NpcId)、UsageCount 字段
- [x] 5.2 创建 `Scripts/Game/Domain/ActionCard/ActionCardMgr.cs`：实现 IDomainMgrBase，单例，维护 `Dictionary<NpcId, List<ActionCard>>`，提供 GiveCard、RemoveCard、GetCards、UseCard 方法
- [x] 5.3 UseCard 方法中：解析 ActionCardDefine → 优先走 StoryIds / 否则 TagBag 匹配 → 调用 StoryMgr.TriggerStory → 若 IsConsumable 则移除实例
- [x] 5.4 在 `WorldMgr.Initialize()` 中注册 ActionCardMgr

## 6. 文档

- [x] 6.1 创建 `Docs/叙事设计.txt`：记录天地人三池设计、StoryBaseDefine 继承体系、双向持有规则、双轨执行架构、ActionCard 系统的完整设计思路
