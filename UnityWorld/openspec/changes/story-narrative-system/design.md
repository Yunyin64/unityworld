...
- `WillPool`：个体持有的 `List<ActionCard>` 实例（通过 ActionCardMgr 管理）
...
### 决策5：ActionCardDefine 双模式指向 Story

**选择**：`ActionCardDefine` 同时支持：
...
- ActionCard 的"消耗"与"保留"逻辑：是否在 ActionCardDefine 上配置 `IsConsumable`？
