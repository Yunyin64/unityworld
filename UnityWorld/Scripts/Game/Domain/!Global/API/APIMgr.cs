...
namespace UnityWorld.Game.Domain
{
    ...
    public class APIMgr : IDomainMgrBase
    {
        ...
        public void ExportLua(string outputDir = @"F:\Openclaw\UnityWorld\UnityWorld\Data\LuaScripts")
        {
            ...
            var groups = new Dictionary<APIType, List<API>>();
            ...
            foreach (var api in _apis.Values)
            {
                ...
                if (!groups.ContainsKey(attr.ApiType))
                    groups[attr.ApiType] = new List<API>();
                groups[attr.ApiType].Add(api);
            }

            foreach (var kv in groups)
            {
                GenerateLuaFile(outputDir, kv.Key, kv.Value);
            }

            LogMgr.Instance.Dbg("[APIMgr] ExportLua 完成，已生成 {0} 个文件到: {1}", groups.Count, outputDir);
        }

        private void GenerateLuaFile(string dir, APIType apiType, List<API> apis)
        {
            var sb = new StringBuilder();
            ...
            string filePath = Path.Combine(dir, $"{apiType}.lua");
            File.WriteAllText(filePath, sb.ToString(), Encoding.UTF8);
